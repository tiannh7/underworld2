...
PetscErrorCode MG_inner_solver_pcmg_setup(KSP_BSSCR *bsscrp_self,
                                          MGContext *mgCtx, KSP ksp_inner,
                                          PC pc_MG, Mat K) {
  PetscFunctionBegin;
...
PetscErrorCode MG_inner_solver_pcmg_shutdown(PC pc_MG) {
  PetscFunctionBegin;
  PetscErrorCode ierr;
  ierr = Stg_PCDestroy(&pc_MG);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
#undef __FUNCT__
#define __FUNCT__ "KSPCycleEffectivenessMonitorAndAdjust"
PetscErrorCode KSPCycleEffectivenessMonitorAndAdjust(KSP ksp, PetscInt n,
                                                     PetscReal rnorm,
                                                     void *_mgctx) {
  PetscErrorCode ierr;
  // PetscViewerASCIIMonitor viewer;
  PetscViewerASCIIMonitor viewer =
      PETSC_VIEWER_STDOUT_(((PetscObject)ksp)->comm);
  ...
  if (n == 0) {
    ...
    mgctx->currentNumberOfSmooths = mgctx->smoothsToStartWith;
  } else if ((n % mgctx->targetCyclesForTenfoldReduction) == 0 &&
             mgctx->currentNumberOfSmooths < mgctx->smoothsMax) {
    mgctx->currentNumberOfSmooths += mgctx->smoothingIncrement;
  }

  ierr = PetscViewerASCIIMonitorCreate(((PetscObject)ksp)->comm, "stdout", 0,
                                       &viewer);
  CHKERRQ(ierr);

  if (mgctx->acceleratingSmoothingMGView) {
    ierr = PetscViewerASCIIMonitorPrintf(
        viewer, "%3D MG smoothing cycles %d [%d] \n", n,
...
double setupMG(KSP_BSSCR *bsscrp_self, KSP ksp_inner, PC pc_MG, Mat K,
               MGContext *mgCtx) {
  double mgSetupTime;
